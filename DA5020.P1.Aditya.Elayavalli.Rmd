...
The data is read into a data frame from CSV format.
...
The data is processed to analyze for outliers, replace null values, and decide what data to include for further analysis.
...
Data is processed further before being joined across data frames. Unique county codes were used to represent each county. The county, age range, program category, and substance group were all used to create separate data frames where each full value has an associated code/abbreviation. The relational nature of the data allowed for the restructuring of our main data frame to include the coded values rather than full descriptions.
...
From the produced graph above, we can see that for minors, the most common substance is marijuana, while in younger adults (18-34) heroin is the most common. For those above the age of 34, alcohol is seen as the most common substance use. These patterns can inform public health strategies and resource allocation for treatment programs. 

## Acknowledgements

All work was completed based on teachings and resources from Modules 1-5 of DA5020 with Professor Ben Tasker, including "R for Data Science" by Wickham and Grolemund (2017). Additionally, we consulted the R documentation for a number of R functions to ensure correct arguments were used. Finally, we utilized ChatGPT, an AI tool developed by OpenAI, for quick ggplot layer reference and function demonstrations for question 4, but not for direct copying of code or problem-specific help, falling far below the 20% usage threshold for this practicum.
